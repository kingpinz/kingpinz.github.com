define(function(require){
	...
	function tab(arg){
		...
		//这里切的是ul所以用children
		var oSublistWrapper=oWrap.children[1];
		var timer=null;
		for(var i=0;i<aHead.length;i++){
			(function(index){
				aContent[i].onmouseover=aHead[i].onmouseover=function(){
					...
					for(var i=0;i<aHead.length;i++){
						aHead[i].className='';
						aContent[i].className='sublistul clearfix';
					}
					this.className='on';
					aContent[index].className='sublistul clearfix show';
					oSublistWrapper.style.display='block';
				};	
				aHead[i].onmouseout=function(){
					timer=setTimeout(function(){
						for(var i=0;i<aHead.length;i++){
							aHead[i].className='';
							aContent[i].className='sublistul clearfix';
							oSublistWrapper.style.display='none';
						}
					},1000)
				};	
